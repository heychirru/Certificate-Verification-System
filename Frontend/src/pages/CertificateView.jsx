...
// IMPORT Auth context and token logic
...
const CertificateView = () => {
  ...
  const handleSaveToWallet = async () => {
    ...
    try {
      ...
      alert("Failed to save to wallet. It might already be saved.");
  ...
  if (status.error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4">
        <svg className="w-12 h-12 text-gh-danger mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
      ...
      <main className="max-w-4xl mx-auto mt-8 px-4">
        <div className="bg-gh-canvas border border-gh-border rounded-md overflow-hidden shadow-sm">
          ...
          <div className="bg-gh-bg border-b border-gh-border px-4 py-3 flex justify-between items-center flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              {/* CONDITIONAL WALLET BUTTON (Only shows if logged in) */}
              {user && (
                <button 
                  onClick={handleSaveToWallet} 
                  disabled={saveState.loading || saveState.success}
                  className="flex items-center gap-1.5 px-3 py-1 bg-gh-canvas border border-gh-border text-xs font-medium text-gh-text rounded-md hover:bg-[#21262d] transition-colors disabled:opacity-50"
                >
                  ...
                  {saveState.success ? 'Saved to Wallet' : saveState.loading ? 'Saving...' : 'Save to Wallet'}
                </button>
              )}

              <button onClick={runSecurityAudit} disabled={audit.loading} className="flex items-center gap-1.5 px-3 py-1 bg-[#1c2128] border border-gh-border text-xs font-medium text-gh-link rounded-md hover:bg-[#2d333b] transition-colors disabled:opacity-50">
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                {audit.loading ? 'Running Audit...' : 'Run Security Audit'}
              </button>
              <button onClick={handleDownload} className="flex items-center gap-1.5 px-3 py-1 bg-gh-green border border-[#2ea043] text-xs font-medium text-white rounded-md hover:bg-gh-greenHover transition-colors">
